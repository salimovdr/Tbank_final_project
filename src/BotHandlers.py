...
class BotHandlers:
    ...
    async def handle_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if caption:
            ...
            try:
                ...
                self.JsonLogger.add(messages_path, str(chat_id), caption, img_id)
            except Exception as e:
            ...
            self.bot.logger.info(str(history) + "  " + str(type(history)))  # debugging REMOVE
            ...
            if res and 'text' in res and res['text']:
                ...
                await update.message.reply_text("No output or 'text' key not found in model response.") # More descriptive error
            ...
            if res and 'image' in res and res['image']:
                ...
                await update.message.reply_text("No output or 'image' key not found in model response.") # More descriptive error
        """
    ...
    async def model_say(self, history: list) -> dict:
        ...
        if model_id in [1, 2]:
            models = {1: self.img_edit_model,
                      2: self.bg_remover}
            img_out = self.model_handler(models[model_id], json_output, user_dir, image, translate=True)
    ...
    def model_handler(self, model, json_output, img_dir, image=None, translate=True):
